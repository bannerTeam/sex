...
class Ajax extends Base
{
    ...
    public function suggest()
    {
        $mid = $this->_param['mid'];
        $wd = $this->_param['wd'];
        if( empty($wd) || !in_array($mid,['1','2','3']) ) {
            return json(['code'=>1001,'msg'=>'参数错误']);
        }
        $mids = [1=>'vod',2=>'art',3=>'topic'];
        $pre = $mids[$mid];
    ...
    public function hits()
    {
        ...
        if(empty($id) ||  !in_array($mid,['1','2','3']) ) {
            return json(['code'=>1001,'msg'=>'参数错误']);
        }
        $mids = [1=>'vod',2=>'art',3=>'topic',4=>'actor',5=>'role'];
        $pre = $mids[$mid];
        $where = [];
        $where[$pre.'_id'] = $id;
    ...
    public function digg()
    {
        ...
        if(empty($id) ||  !in_array($mid,['1','2','3','4']) ) {
            return json(['code'=>1001,'msg'=>'参数错误']);
        }

        $mids = [1=>'vod',2=>'art',3=>'topic',4=>'actor',5=>'role',6=>'comment'];
        $pre = $mids[$mid];
        $where = [];
        $where[$pre.'_id'] = $id;
